...
//静的メンバ変数宣言-------------------
CObjectX::MODELX CCar::m_model[CCar::TYPE_MAX] = {};
const char *CCar::m_apFilename[CCar::TYPE_MAX] =
{
	"data/MODEL/deliveryCar000.x",
};
//=====================================
// コンストラクタ・デストラクタ
//=====================================
CCar::CCar(int nPriority = 3) : CObjectX(nPriority)
{
	m_fHueOrg = 0.0f;
	m_fHue = 0.0f;
}
...
// 生成処理
//=====================================
CCar *CCar::Create(D3DXVECTOR3 pos, D3DXVECTOR3 rot, float fWidth, float fHeight, CCar::TYPE type)
{
	CCar *pObjectBlock;

	//2Dオブジェクトの生成
	pObjectBlock = new CCar();
...
// ポリゴンのテクスチャ処理
//=====================================
HRESULT CCar::Load(void)
{
	LPDIRECT3DDEVICE9 pDevice = CManager::Get()->GetRenderer()->GetDevice();

	D3DXMATERIAL *pMat;

	for (int nCnt = 0; nCnt < CCar::TYPE_MAX; nCnt++)
	{
		//xファイルの読み込み
