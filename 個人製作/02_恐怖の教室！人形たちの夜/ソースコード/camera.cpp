...
void InitCamera(void)
{
	...
	g_camera.rot = D3DXVECTOR3(0.0f, -0.0f, 1.57f);
	g_camera.rotOld = D3DXVECTOR3(0.0f, -0.0f, 1.57f);
...
void UpdateCamera(void)
{
	...
	if (nRStickLR != 0)
	{
		g_camera.rot.y += 0.02f * ((float)nRStickLR * 0.001f);
	}
	if (nRStickUD != 0)
	{
		g_camera.rot.z += 0.02f * ((float)nRStickUD * 0.001f);
	}

	RotCamera();

	if (GetGameState() == GAMESTATE_BOSS)
	{
		g_camera.posR.x = 0.0f;
		g_camera.posR.y = 800.0f;
		g_camera.posR.z = 200.0f;
	}
}
...
void UpdateCameraTitle(void)
{
	...
	g_camera.rot = D3DXVECTOR3(0.0f, -1.0f, 1.5f);
	g_camera.rotOld = D3DXVECTOR3(0.0f, -1.0f, 1.5f);
...
void RotCamera(void)
{
	...
	fLengthCamera = 130.0f;

	g_camera.posRDest.x = pPlayer->pos.x - (sinf(pPlayer->rot.y) * 0.0f);
	g_camera.posRDest.y = pPlayer->pos.y + 70.0f;
	...
	if (g_camera.posVDest.y < 0.0f)
	{
		g_camera.rot.z = g_camera.rotOld.z;
		g_camera.posVDest.y = g_camera.posVOld.y;
	}

	if (g_camera.posVDest.y > 140.0f)
	{
		g_camera.rot.z = g_camera.rotOld.z;
		g_camera.posVDest.y = g_camera.posVOld.y;
	}

	if (CollisionMeshWall(&g_camera.posV, &g_camera.posR, &g_camera.posV) == true)
	{
...
void RotCameraTitle(void)
{
	...
	fLengthCamera = 150.0f;

	g_camera.posRDest.x = pPlayer->pos.x - (sinf(pPlayer->rot.y) * 0.0f);
	g_camera.posRDest.y = pPlayer->pos.y + 30.0f;
	g_camera.posRDest.z = pPlayer->pos.z - (cosf(pPlayer->rot.y) * 0.0f);
