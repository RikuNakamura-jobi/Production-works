...
//マクロ定義
#define NUM_STAGESTART (5)												//ボス警告の数
...
void InitStagestart(void)
{
	...
	D3DXCreateTextureFromFile(pDevice,
		"data\\TEXTURE\\stagecall000.png",
		&g_apTextureStagestart[0]);

	D3DXCreateTextureFromFile(pDevice,
		"data\\TEXTURE\\stagecall001.png",
		&g_apTextureStagestart[1]);

	D3DXCreateTextureFromFile(pDevice,
		"data\\TEXTURE\\stagecall002.png",
		&g_apTextureStagestart[2]);

	D3DXCreateTextureFromFile(pDevice,
		"data\\TEXTURE\\stagecall003.png",
		&g_apTextureStagestart[3]);

	D3DXCreateTextureFromFile(pDevice,
		"data\\TEXTURE\\stagecall004.png",
		&g_apTextureStagestart[4]);
...
void UpdateStagestart(void)
{
	...
	{
		if (g_nTimerStagestart < 240.0f && g_nTimerStagestart >= 60.0f)
		{
		...
		else if (g_nTimerStagestart >= 240.0f)
		{
			...
			else if (g_fCntStagestart <= 0.0f)
			{
				...
				SetPlayerState(PLAYERSTATE_NORMAL, 0);
			}
		}
...
void DrawStagestart(void)
{
	...
	{
		//テクスチャの設定
		pDevice->SetTexture(0, g_apTextureStagestart[nStageType]);

		//ポリゴンの描画
		pDevice->DrawPrimitive(D3DPT_TRIANGLESTRIP, (nStageType) * 4, 2);
	}
}
