...
void InitMeshWall(void)
{
	...
	for (int nCnt = 0; nCnt < MAX_MESHWALL; nCnt++)
	{
		...
		g_meshWall[nCnt].fWallLength = 2700.0f;

		g_meshWall[nCnt].posVec[0].x = g_meshWall[nCnt].pos.x + (-2700.0f * cosf(g_meshWall[nCnt].rot.y));
		g_meshWall[nCnt].posVec[0].y = 0.0f;
		g_meshWall[nCnt].posVec[0].z = g_meshWall[nCnt].pos.z + (2700.0f * sinf(g_meshWall[nCnt].rot.y));

		g_meshWall[nCnt].posVec[1].x = g_meshWall[nCnt].pos.x + ((-2700.0f + (2700.0f * g_meshWall[nCnt].Numx)) * cosf(g_meshWall[nCnt].rot.y));
		g_meshWall[nCnt].posVec[1].y = 0.0f;
		g_meshWall[nCnt].posVec[1].z = g_meshWall[nCnt].pos.z + ((2700.0f - (2700.0f * g_meshWall[nCnt].Numx)) * sinf(g_meshWall[nCnt].rot.y));
	}
	...
	for (int nCnt1 = 0; nCnt1 < MAX_MESHWALL; nCnt1++)
	{
		...
		{
			//頂点座標の設定
			pVtx[0].pos.x = -2700.0f + (2700.0f * (nCnt % (g_meshWall[nCnt1].Numx + 1)));
